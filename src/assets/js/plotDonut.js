...
function DonutCharts(idDiv) {
	...
	this.create = function(dataset, idDiv) {
		var $charts = $('#' + idDiv);
...
function createDonutCharPublicAndInternalInterface(data){
    if(data){
        var idDiv = 'chart3-area-plot';
        var data = createDataFormatDonut(data);
        var donuts = new DonutCharts(idDiv);
        donuts.create(data, idDiv);   
    }
}
